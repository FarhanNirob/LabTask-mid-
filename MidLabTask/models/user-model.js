...
module.exports ={
	...
	getByType: function(uname, callback){
		var sql = "select type from employee where username=? and password=?";
		db.getResult(sql, [uname], function(result){
			if(result.length > 0){
				callback(result[0]);
			}else{
	...
	insert: function(user, callback){
		...
		db.execute(sql, [null, employee.employeename, employee.password, employee.type], function(status){
			if(status){
				callback(true);
			}else{
				callback(false);
	...
	update: function(user, callback){
		var sql = "update employee set employeename=?,contactno=?,username=? password=?, where id=?";
